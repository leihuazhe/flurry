...
import com.yunji.flurry.core.RegistryMetadataClient;
import com.yunji.flurry.metadata.OptimizedService;
import com.yunji.flurry.metadata.jmx.JmxCache;
...
public class ExportServiceManager implements RegistryListener, ConfigListener {
     ...
     */
    private static final ConcurrentMap<String, com.yunji.flurry.metadata.OptimizedService> serviceMetadataMap = new ConcurrentHashMap<>(256);
    /**
     * 暂存最近一次的服务白名单信息.
     */
    ...
    public com.yunji.flurry.metadata.OptimizedService getMetadata(String interfaceName, String version) {
        return getMetadata(interfaceName);
    }

    public com.yunji.flurry.metadata.OptimizedService getMetadata(String interfaceName) {
        return serviceMetadataMap.get(interfaceName);
    }

    public Map<String, com.yunji.flurry.metadata.OptimizedService> getServiceMetadataMap() {
    ...
    public synchronized void notify(Properties properties) {
        ...
        for (String serviceName : referServiceList) {
            ...
            if (StringUtils.isNotEmpty(serviceName)) {
                com.yunji.flurry.metadata.OptimizedService service = serviceMetadataMap.get(serviceName);
                if (service == null) {
                    synchronized (this) {
    ...
    private void refreshMetadata(String path, List<URL> providerURLs) {
        try {
            if (!providerURLs.isEmpty()) {
                ...
                com.yunji.flurry.metadata.OptimizedService optimizedService = MetadataUtil
    ...
    protected void refreshMetadataAsync(String path, List<URL> providerURLs) {
        if (!providerURLs.isEmpty()) {
            ...
            CompletableFuture<com.yunji.flurry.metadata.OptimizedService> resultFuture
